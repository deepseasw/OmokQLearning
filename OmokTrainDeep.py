...
W_conv1 = tf.Variable(tf.truncated_normal([filter1_size, filter1_size, 1, filter1_num], stddev = 0.1))
h_conv1 = tf.nn.conv2d(x_image, W_conv1, strides = [1, 1, 1, 1], padding = 'SAME')
b_conv1 = tf.Variable(tf.constant(0.1, shape = [filter1_num]))
...
filter2_num = 64
filter2_size = 5
W_conv2 = tf.Variable(tf.truncated_normal([filter2_size, filter2_size, filter1_num, filter2_num], stddev = 0.1))
h_conv2 = tf.nn.conv2d(h_pool1, W_conv2, strides = [1, 1, 1, 1], padding = 'SAME')
b_conv2 = tf.Variable(tf.constant(0.1, shape = [filter2_num]))
...
w0 = tf.Variable(tf.zeros([units2_num, nbActions]))
b0 = tf.Variable(tf.zeros([nbActions]))
output_layer = tf.matmul(hidden2_drop, w0) + b0
